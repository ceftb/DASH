...
% Agent to explore interaction of password use and complexity with
% BCMA model. Copied from bcma.pl 1/26/14 with some aspects of BCMA
% protocol simplified and password modeling added.
...
% In the current agent, the nurse skips scanning when p(document| no
% scan) >= 0.95
% ]] end bcma comments.
...
primitiveAction(rememberPassword).
primitiveAction(enterPassword).
...
% Add a delivery step for each patient
buildDeliveryList([],[]).
buildDeliveryList([First|Rest],[deliverMeds(First)|RestDeliveries]) :- buildDeliveryList(Rest,RestDeliveries).

% Top level goal for a patient retrieves the protocol and decides whether to perform each step in turn
...
% Otherwise, projection should not fail, or so will comparing the plans, so specify that nothing happens.
