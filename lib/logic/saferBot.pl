...
% Reporting on the state of the agent
state(S) :- messageSent(M), sformat(S, 'I sent message ~w', [M]).
state(S) :- privacyTool(T), softwareInstalled(T), sformat(S, 'I have installed ~w', [T]).
state(S) :- privacyTool(T), haveMetaData(T), sformat(S, 'I have acquired ~w', [T]).
state('I am a SAFER agent').
