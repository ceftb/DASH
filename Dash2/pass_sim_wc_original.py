...
class PasswordAgent(DASHAgent):
    ...
    parameters = [Parameter('initial_belief', default=0.65, range=(0, 1)),  # initial strength of beliefs
                  ...
                  Parameter('cog_burden_type', value_set=['word-count', 'levenshtein', 'both'], default='word-count'),
                  Parameter('hacker_guesses_per_s', default=100000000),
                  Parameter('hacker_attack_s', default=3600),
                  Parameter('stolen_password_risk', default=0.25),
                  Parameter('reuse_attack_risk', default=0.25)
                  ]
    ...
    def __init__(self):
        ...
        # dict where service is key and written pw is value
        self.service_written_pw_dict = {}
...
""")
    ...
    def setup_account(self, (goal, service_type_var, service_var)):
        ...
        password, written = self.choose_password(username, requirements)
        ...
        if status == 'success':
            ...
            if password is not written:
                ...
                self.service_written_pw_dict[service] = password
    ...
    def reset_password(self, (goal, service)):
        ...
        new_password, is_written = self.choose_password(username)

        status_response = self.sendAction('resetPassword', [service, username, old_password, new_password])

        if status_response[0] == 'success':
            self.num_resets += 1
            if service in self.service_written_pw_dict and self.service_written_pw_dict[service] == old_password:
                self.service_written_pw_dict[service].pop(service)
            if is_written:
                self.service_written_pw_dict[service] = new_password
    ...
    def cog_load(self, password_set):
        if self.cog_burden_type == 'both':
            return 0.5 * levenshtein_set_cost(password_set + [""]) + 0.5 * \
...
def expected_number_of_sites(reuses):
    total = sum(i * v for i, v in reuses)  # each pair means v username/pass8word combinations are used i times
