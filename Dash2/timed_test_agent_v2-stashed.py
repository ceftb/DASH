...
transient doWork
    """)
        ...
        self.primitiveActions([
                ...
                ('eat', self.eat_fail),
                ('sleep', self.sleep),
                ('chooseMeal', self.choose_meal)
                ])

    def choose_meal(self, (goal, meal_var)):
        return [{meal_var: meal} for meal in ["_breakfast", "_lunch", "_dinner"]]
    ...
    def eat_fail(self, (goal, meal)):
        print 'eating', meal
        (a, b) = (8, 9) if meal == "_breakfast" else ((11, 14) if meal == "_lunch" else (18, 20))
        eat_time = 24 * math.floor(self.time/24) + random.randint(a, b)
        self.sendAction(goal + "(" + meal + ")", [], eat_time)
        self.time = eat_time
        return []
