...
CONFIG_FILE_PATH=./deter.conf # path is relative to $WEBDASH_CLONE/Dash2/core/
...
function perform_action_on_all_nodes {
	...
	do
        ...
        do
            ...
            ssh ${DASH_NODES[$ID-1]} "tmux new-session -d bash $WEBDASH_CLONE/Dash2/core/$SCRIPT_NAME $WORKER_ID ${ZK_MAP[${DASH_NODES[$ID-1]}]}:2181 $WEBDASH_CLONE $ACTION"
        done
