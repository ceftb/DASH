...
goalRequirements(doOrdinaryWork, [updateAgentState(unexpectedness,0.05)]) :-
	check(agentState(unexpectedness,U)), U > 0.8, response(bugFixed), !. % Once the IT agent fixes it, we're good.
...
goalRequirements(doOrdinaryWork, [callPerson(IT, taskFixPCRequest(ID))]) :-
	check(agentState(unexpectedness,U)), U > 0.8, id(ID), myIT(IT), !.

goalRequirements(doOrdinaryWork, [checkInsertedValue])
        :- checkingValues, !.    % Once you start checking a value, stick with it.

goalRequirements(doOrdinaryWork, [checkInsertedValue])
        :- check(agentState(unexpectedness,U)), U =< 0.8, 
	   insertedValues(_,_,_,_), checkSpreadsheetChance(P), N is random(100)/100, N < P, myAssert(checkingValues), !.
...
goalRequirements(doOrdinaryWork, [doNothing]).  % If no work to do or unable.
...
goalRequirements(openBrowser,	[printMessage('Browser on top\n')])
...
goalRequirements(insertValues(Spreadsheet, Row, Col), [toolsForWorkflow_spreadsheetSetValue(Value, Row, Col)])
	:- onSpreadsheet(Spreadsheet), valueToSet(Spreadsheet, Row, Col, Value), !.

% It never does the 'continue' goal, it just stops it from assuming it's finished. Need a better syntax.
goalRequirements(insertValues(Spreadsheet, Row, Col), [toolsForWorkflow_spreadsheetSwitchSheet(Spreadsheet),continue]) :- !.

valueToSet(Spreadsheet, Row, Col, Value) :- check(cellVal(Spreadsheet, Row, Col, Value)), ranf(Rand), 0.25 < Rand, !.
valueToSet(Spreadsheet, Row, Col, Value) :- check(cellVal(Spreadsheet, Row, Col, Value)), check(unexpectedness(U)), U < 0.5, !.
valueToSet(Spreadsheet, Row, Col, NewValue) :- check(cellVal(Spreadsheet, Row, Col, Value)), NewValue is Value*10.
...
goalRequirements(reportAgent, RequirementList)
	:-	findall(insertedValues(Spreadsheet, Row, Col, Value), insertedValues(Spreadsheet, Row, Col, Value), RawList),
		...
		id(ID),
		append([printMessage('Agent ~w now knows:\n', [ID])], TempReqList, RequirementList).
...
createListInsertedValues([insertedValues(Spreadsheet, Row, Col, Val)|T], InsertedValues)
	:-	createListInsertedValues(T, TempInsertedValues),
		append([printMessage('Inserted ~w in ~w spreadsheet at row ~w col ~w\n', [Val, Spreadsheet, Row, Col])], 
		TempInsertedValues, InsertedValues).
...
% Special 'continue' goal to stop the planner from closing off a goal
goalRequirements(continue, [continueAction]).
...
updateBeliefs(computer_applicationSwitch(App),1) :- % Successful switch of focus
	...
	myRetract(onSpreadsheet(_)),
	execute(updateAgentState(tiredness, App)), 
	execute(updateAgentState(unexpectedness, 0.05)), 
	!.

updateBeliefs(computer_applicationSwitch(_),0) :-   % Failed
	execute(factorAgentState(unexpectedness, 1, 0.1)), % was 0.5: 'expectedness' halves each time, this is 1 - expectedness
	!.

updateBeliefs(computer_applicationOpen(_),0) :-   % Failed
	execute(factorAgentState(unexpectedness, 1, 0.1)), 
	!.

updateBeliefs(toolsForWorkflow_spreadsheetSwitchSheet(Spreadsheet),1) :-
        format('Switched to spreadsheet ~w\n', [Spreadsheet]), myRetractall(onSpreadsheet(_)), myAssert(onSpreadsheet(Spreadsheet)).

updateBeliefs(toolsForWorkflow_spreadsheetSwitchSheet(Spreadsheet),0) :-
        format('Failed to switch to ~w\n', [Spreadsheet]).

updateBeliefs(toolsForWorkflow_spreadsheetSetValue(Value, Row, Col),1) :-
        onSpreadsheet(Spreadsheet), myAssert(insertedValues(Spreadsheet, Row, Col, Value)),
	execute(updateAgentState([[tiredness, toolsForWorkflow_spreadseetSetValue], [unexpectedness, 0.2], [frustrated, 0.01]])),
	format('Set value ~w', [Value]).

updateBeliefs(toolsForWorkflow_spreadsheetSetValue(Value, Row, Col),0) :-
        format('Warning: was not able to insert value ~w in row ~w\n', [Value, Row]),
	execute(updateAgentState([[tiredness, toolsForWorkflow_spreadseetSetValue], [unexpectedness, 0.9], [frustrated, 0.01]])).
...
executable(rememberInserted(Spreadsheet, Row, Col, Value)).
execute(rememberInserted(Spreadsheet, Row, Col, Value)) :- myAssert(insertedValue(true)), myAssert(insertedValues(Spreadsheet, Row, Col, Value)).
...
executable(finishCheckingExecutable).
execute(finishCheckingExecutable) :- myRetract(checkingValues).
...
% Part of the continuation idea
primitiveAction(continueAction).
...
subGoal(continue).  % Trying something that forces the planner to mark the goal as unsolved
