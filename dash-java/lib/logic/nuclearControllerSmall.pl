...
goalRequirements(plantOk, [check(coolantTemperature)]) 
    :- value(coolantTemperature, unknown), !.
goalRequirements(plantOk, [doNothing]) :- ok(coolantTemperature).
goalRequirements(plantOk, [fixCoolantTemperature]) 
    :- high(coolantTemperature), assert(targetField(coolantTemperature)).
...
goalRequirements(fixCoolantTemperature, [check(waterPressure)]) 
    :- value(waterPressure, unknown), !.
